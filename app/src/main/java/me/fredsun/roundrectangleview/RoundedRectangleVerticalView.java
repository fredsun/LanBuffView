...
public class RoundedRectangleVerticalView extends View {
    ...
    public RoundedRectangleVerticalView(Context context) {
        this(context, null);
    }

    public RoundedRectangleVerticalView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RoundedRectangleVerticalView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        path = new Path();
        pathMeasure = new PathMeasure();
        //构造时开启抗锯齿
        paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        paint.setColor(Color.parseColor("#ff8d0b"));
...
//        pathMeasure.getPosTan(mAnimatorValue * pathMeasure.getLength(), pos, tan);
        ...
        canvas.drawLine(mWidth/2, mHeight-mWidth/2, mWidth/2, mWidth/2, paint);

        paint.setXfermode(mXfermode);

        //画动态柱状图SRC
        paint.setColor(progressColor);
        canvas.drawLine(mWidth/2, mHeight+mWidth/2, mWidth/2, mHeight+mWidth/2-mAnimatorValue*mHeight, paint);
